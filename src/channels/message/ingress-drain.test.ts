...
describe("channel ingress drain", () => {
  ...
  it("throws IngressAdoptionLostError when onAdopted races supersede", async () => {
    await withTempState(async (stateDir) => {
      ...
      expect(lateAdoptError).toBeInstanceOf(IngressAdoptionLostError);
      expect((lateAdoptError as IngressAdoptionLostError).code).toBe("superseded");
      drain.dispose();
    });
  });
  ...
  it("holds claim ownership when tombstone complete keeps failing", async () => {
    await withTempState(async (stateDir) => {
      ...
      for (let i = 0; i < INGRESS_TOMBSTONE_RETRY_MAX_ATTEMPTS; i += 1) {
  ...
  it("throws IngressAdoptionLostError when complete returns false (lease reclaimed)", async () => {
    await withTempState(async (stateDir) => {
      ...
      await drain.drainOnce();
      await drain.waitForIdle();
      expect(adoptError).toBeInstanceOf(IngressAdoptionLostError);
      expect((adoptError as IngressAdoptionLostError).code).toBe("reclaimed");
  ...
  it("exports default adoption stall matching Telegram product default", () => {
    ...
    expect(INGRESS_TOMBSTONE_RETRY_MAX_ATTEMPTS).toBe(8);
  });
});
