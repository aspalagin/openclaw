...
```
    ...
    Local media: with rich messages on, `<img>`, `<video>`, and `<audio>` sources (standalone, inside `<figure>`, `<tg-collage>`, or `<tg-slideshow>`, or as Markdown images) accept absolute local paths as well as HTTPS URLs. Paths go through the same outbound media policy as attachments (allowed local roots, `mediaMaxMb`): photos up to 10 MB within Telegram's photo limits, videos, and audio are uploaded inline with the message. GIFs and other local types are rejected for inline use; send them as attachments. Local photo, video, and audio attachments delivered together with rich text are embedded in the same message, and any other attachments follow in their original order. If Telegram rejects the rich upload, the text is delivered as plain messages and the local files are re-sent through the regular media path.
