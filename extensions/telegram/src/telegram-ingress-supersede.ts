...
export async function isTelegramSpooledUpdateSenderAuthorized(
  ...
  const groupAllowContext = await resolveTelegramGroupAllowFromContext({
    ...
    groupAllowFrom,
    // Test seam: inject preloaded store entries; live path uses default reader.
    ...(auth.storeAllowFrom !== undefined
      ? {
          readChannelAllowFromStore: async () => auth.storeAllowFrom ?? [],
        }
      : {
          readChannelAllowFromStore: (channel, env, accountId) =>
            readChannelAllowFromStore(channel, env ?? auth.env ?? process.env, accountId),
        }),
